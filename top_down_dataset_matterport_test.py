...
class MyDataset(Dataset):
    ...
    def __init__(self):
        ...
        open_file = open(os.path.join(dataset_path, "test_data.json"))
        ...
        for camera in data:
            camera_data = {
                ...
                "render": "Generate an ultra-high-definition panoramic depth map of an indoor scene using the provided condition input. The condition consists of a composite image: a coarse RGB image and a corresponding coarse depth map obtained from density estimation. Utilize the coarse depth map to reconstruct the structural layout of the room, ensuring accurate depth continuity and geometric consistency. Use the coarse RGB image as a reference to refine the depth estimation of furniture, floor, and other indoor elements.",
        ...
        json.dump(self.data, open(os.path.join(dataset_path, "data_processed_test.json"), 'w'))
    ...
    def __getitem__(self, idx):
        ...
        return dict(jpg=source, txt=prompt, hint=detected_map, location=location,
                    ...
                    top_down_image=cv2.imread(source_filename, cv2.IMREAD_COLOR), dataset_name='matterport')